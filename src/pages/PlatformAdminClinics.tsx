...
export default function PlatformAdminClinics() {
  ...
  const fetchClinics = async () => {
    ...
    if (!error && data) setClinics(data as unknown as Clinic[]);
    setLoading(false);
  };
  ...
  const filtered = clinics.filter(c => c.status === tab);
    ...
    <PlatformAdminLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Review, approve, or reject clinic registrations.</p>
        </div>

        <Tabs value={tab} onValueChange={setTab}>
          <TabsList>
            <TabsTrigger value="pending" className="gap-1.5">
              <Clock className="h-3.5 w-3.5" /> Pending ({clinics.filter(c => c.status === 'pending').length})
            </TabsTrigger>
            <TabsTrigger value="approved" className="gap-1.5">
              <CheckCircle className="h-3.5 w-3.5" /> Approved ({clinics.filter(c => c.status === 'approved').length})
            </TabsTrigger>
            <TabsTrigger value="rejected" className="gap-1.5">
              <XCircle className="h-3.5 w-3.5" /> Rejected ({clinics.filter(c => c.status === 'rejected').length})
            </TabsTrigger>
          </TabsList>
          ...
          <TabsContent value={tab} className="mt-4">
            {loading ? (
              ...
              <Card className="border-dashed">
                <CardContent className="py-12 text-center text-muted-foreground">No {tab} clinics found.</CardContent>
              </Card>
            ) : (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3">
                        ...
                        {clinic.status === 'pending' && (
                          <div className="flex gap-2 shrink-0">
                            <Button size="sm" className="gap-1.5" disabled={actionLoading === clinic.id} onClick={() => updateStatus(clinic.id, 'approved')}>
                              {actionLoading === clinic.id ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle className="h-3.5 w-3.5" />}
                              Approve
                            </Button>
                            <Button size="sm" variant="destructive" className="gap-1.5" disabled={actionLoading === clinic.id} onClick={() => updateStatus(clinic.id, 'rejected')}>
                              <XCircle className="h-3.5 w-3.5" /> Reject
                            </Button>
                          </div>
                        )}
                        {clinic.status === 'rejected' && (
                          <Button size="sm" variant="outline" className="gap-1.5 shrink-0" disabled={actionLoading === clinic.id} onClick={() => updateStatus(clinic.id, 'approved')}>
